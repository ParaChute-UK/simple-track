...
def track_storms(
    ...
    ###################################################
        ...
        for corx in range(0, int(np.size(xint, 0))):
            ...
            for cory in range(0, int(np.size(xint, 1))):
                ...
                if (
                    ...
                    buu[corx, cory] = dx
                    bvv[corx, cory] = (
                        dy  ## indices are upside down so need minus to get real-world dy-velocity
                    )
        ...
        for ns in range(numstorms):
            ...
            ###################################################
            qhist = (np.histogram(QuvL[np.where(StormLabels == jj)], qbins))[0][
                :
            ] / float(StormData[ns].area) + (
            ...
            if np.max(qhist[1:]) < lapthresh:
                ...
                qhist = (np.histogram(QuvL[np.where(newblob == 1)], qbins))[0][
                    :
                ] / float(StormData[ns].area) + (
            ...
            if np.max(qhist[1:]) >= lapthresh:
                ...
                ###################################################
                    ...
                    lapdist = np.sqrt(
                        ...
                        + (StormData[ns].centroidy - OldStormData[zindex].centroidy)
                        ** 2
                    )
                    StormData[ns].inherit_properties(
                        jj,
        ...
        for ns in range(len(StormData)):
            ...
            for kkind in range(np.size(wassep)):
                if not kkind == kkmax:
                    ...
                    lifearray[np.where(StormLabels == wasind[0][kkind] + 1)] = (
                        StormData[wasind[0][kkmax]].life
                    )
                    newwas = newwas + 1
